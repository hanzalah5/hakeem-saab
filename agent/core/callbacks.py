...
async def load_root_prompt(callback_context: CallbackContext) -> None:
    """Inject the coordinator's routing prompt into session state."""
    callback_context.state["system_prompt"] = load_prompt("root_router")
...
async def save_session_to_memory(callback_context: CallbackContext) -> None:
    ...
    Failures are logged but never raised — a memory-write hiccup should
