"""SQL-backed session persistence for the FemVerse agent.

ADK separates two persistence layers:

- **Session** (this module) — short-term per-conversation events + state,
  read/written every turn.
- **Memory** (`agent.memory.service`) — long-term, cross-session extracted
  facts.

`build_session_service()` is invoked by the runner. It looks for a
SQLAlchemy URL in this order:

1. `SESSION_DB_URL` environment variable (via `agent.config.settings`).
...
def get_database_url() -> str:
    ...
    **Plug in your SQL database here.** Examples of valid return values:
    ...
    in your ``.env`` file — the resolution order in
